...
// TypeScript file
namespace app.spy {
    interface ISpyGameData {
        txt: string
    }
    export class SpyGameDlg extends ui.Dialog {
        imgTouch: eui.Image;
        tbrKings: eui.TabBar;
        ...
        updateView() {
            ...
            this.tbrKings.selectedIndex = model - 5;
        }
        ...
        onTbrKingsItemTap() {
            let data = this.tbrKings.selectedItem as ISpyGameData;
            this.lblSelectedModel.text = data.txt;
            manager.spy.setPlayModel(+data.txt);
            this.hideDropList();
        }
